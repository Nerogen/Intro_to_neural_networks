...
from utils.util import find_Eq, user_input, make_cubes_and_vectors, \
    process_compression_bytes_and_made_image, pickle_dump_to_file, learning_with_numpy, open_and_process_image
...
def main() -> None:
    ...
    first_weight = numpy.array([[random.uniform(left_max_weight, right_max_weight) for _ in range(p)] for _ in
                                range(len(vectors[0]))])
    second_weight = first_weight.T
    delta_X, Yi, X_i, delta_X, first_weight, second_weight = learning_with_numpy(vectors, first_weight, second_weight)
    count_rounds = 0
    while find_Eq(delta_X) > e:
        print(f'Round: {count_rounds} | Error: {find_Eq(delta_X)} > {e}')
        count_rounds += 1
        delta_X, Yi, X_i, delta_X, first_weight, second_weight = learning_with_numpy(vectors, first_weight,
                                                                                     second_weight)
